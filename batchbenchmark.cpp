...
struct SplayTree {
    ...
    //  max (4, log2(size of the tree) * (2.0))
...
int main() {
    ...
    string file_type = "strict_cache_access";
    ifstream inputFile("tests/" + file_type + ".txt"); // Assuming input file name is input.txt
    ...
    // string outName = file_type + ".csv";
    // ofstream outputFile(outName); // Output CSV file

    // outputFile << fixed << setprecision(10);
    // outputFile << "splay_depth,splay_count,avg_runtime,avg_root_distance\n";
    auto evaluate = [&](int depth) {
        mySet.SPLAY_DEPTH = depth;
        mySet.splay_count = 0;
        mySet.total_depth = 0;
